...
class RoIHeads(nn.Module):
    ...
    def forward(self,
        ...
        if self.training:
            proposals, matched_idxs, all_labels, regression_targets = self.select_training_samples(proposals, targets)
            ...
            for bl in all_labels:
                labels_per_batch = []
                coarse_labels_per_batch = []
                for l in bl:  
                    labels_per_batch.append(l[0])
                    coarse_labels_per_batch.append(l[1])
                labels.append(torch.tensor(labels_per_batch).to(device))
                coarse_labels.append(torch.tensor(coarse_labels_per_batch).to(device))
        ...
        coarse_box_features = self.box_roi_pool(coarse, proposals, image_shapes)
        
        box_features = self.box_head(box_features)
        coarse_box_features = self.box_head(coarse_box_features)
        
        class_logits, box_regression = self.box_predictor(box_features)
        coarse_class_logits, coarse_box_regression = self.box_predictor(coarse_box_features)
        ...
        if self.training:
            ...
            coarse_loss_classifier, loss_box_reg = fastrcnn_loss(
                coarse_class_logits, coarse_box_regression, coarse_labels, regression_targets)
            
            losses = {
                "loss_classifier": loss_classifier+coarse_loss_classifier,
